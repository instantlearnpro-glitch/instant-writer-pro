...
export const getElementSignature = (element: HTMLElement, ignoreTextLength = false): string => {
  ...
  let contentPattern = 'any';
  if (!ignoreTextLength) {
    const text = element.textContent?.trim() || '';
    const isNumeric = /^\d+$/.test(text);
    const isShortText = text.length < 10;
    contentPattern = isNumeric ? 'numeric' : (isShortText ? 'short' : 'long');
  }
...
export const findSimilarElements = (
  ...
  allElements.forEach((el) => {
    ...
    const elSignature = getElementSignature(htmlEl, ignoreTextLength);
